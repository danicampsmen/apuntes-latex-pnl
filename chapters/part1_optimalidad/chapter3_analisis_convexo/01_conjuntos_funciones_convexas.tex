...
\begin{proposition}\label{prop:dual_cono_convexo_cerrado}
...
\begin{definition}[Cápsula cónica o envoltura cónica]\label{def:capsula_conica}
...
\begin{proposition}[Cápsula cónica de un conjunto convexo]\label{prop:capsula_conica_convexa}
...
\begin{proof}
    ...
    Observamos que $C$ es un cono que contiene a $D$ (basta tomar $\alpha = 1$). Por lo tanto, si demostramos que $C$ es convexo, $C$ será un cono convexo que contiene a $D$, de donde la inclusión $\text{cone } D \subset C$ será obvia por la \cref{def:capsula_conica}.
...
\begin{theorem}[Direcciones viables y cono tangente de un conjunto convexo]\label{thm:capsula_conica_convexa}
...
\begin{proof}
    Es fácil ver que el conjunto $D - \{\bar{x}\}$ es convexo. Por lo tanto, por la \cref{prop:capsula_conica_convexa}:
    \begin{equation}
        \text{cone}(D - \{\bar{x}\}) = \{d \in \Rn \mid d = \alpha(x - \bar{x}), \, x \in D, \, \alpha \in \R_+\}. \label{eq:cono_trasladado}
    \end{equation}
...
\begin{proposition}\label{prop:dual_clausura_cono}
...
\begin{proof}
    ...
    Sean $y \in K^*$ y $d \in \cl K$ cualesquiera. Existe $\{d^k\} \subset K$ tal que $\{d^k\} \to d$. Tenemos que $\interno{y}{d^k} \leq 0$ para todo $k$. Pasando al límite cuando $k \to \infty$, obtenemos que $\interno{y}{d} \leq 0$. Por lo tanto, $y \in (\cl K)^*$, es decir, $K^* \subset (\cl K)^*$. Ahora, la última afirmación se sigue del \cref{thm:cono_tangente_convexo}.
\end{proof}

Otra noción útil en Análisis Convexo es la de cono normal.

\begin{definition}[Cono normal]\label{def:cono_normal}
...
\begin{theorem}[El cono normal es el dual del cono tangente]\label{thm:normal_dual_tangente}
...
\begin{proof}
    Las primeras dos igualdades ya fueron probadas en la \cref{prop:dual_clausura_cono}. Supongamos que $y \in (\text{cone}(D - \{\bar{x}\}))^*$, es decir, $\interno{y}{d} \leq 0$ para todo $d \in \text{cone}(D - \{\bar{x}\})$. En particular, $\interno{y}{x - \bar{x}} \leq 0$ para todo $x \in D$, es decir, $y \in \mathcal{N}_D(\bar{x})$.
...
\begin{theorem}[Condición necesaria de primer orden]\label{thm:cn1_convexo}
    ...
    \begin{equation}
        \interno{f'(\bar{x})}{x - \bar{x}} \geq 0 \quad \forall x \in D, \label{eq:desigualdad_variacional_convexa}
    \end{equation}
    o, equivalentemente,
    \begin{equation}
        -f'(\bar{x}) \in \mathcal{N}_D(\bar{x}). \label{eq:gradiente_cono_normal}
    \end{equation}
\end{theorem}

\begin{proof}
    Por las condiciones primales de primer orden del Capítulo 1, $\interno{f'(\bar{x})}{d} \geq 0$ para todo $d \in \mathcal{B}_D(\bar{x})$. Por el \cref{thm:cono_tangente_convexo}, tenemos que:
    \[
        \{d \in \Rn \mid d = x - \bar{x}, \, x \in D\} \subset \text{cone}(D - \{\bar{x}\}) \subset \mathcal{B}_D(\bar{x}),
    \]
    lo que implica \eqref{eq:desigualdad_variacional_convexa}. Por la \cref{def:cono_normal}, \eqref{eq:desigualdad_variacional_convexa} y \eqref{eq:gradiente_cono_normal} son equivalentes.
\end{proof}

Si la función $f$ es convexa, la condición necesaria de optimalidad dada en el \cref{thm:cn1_convexo} también es suficiente.

\begin{definition}[Funciones convexas, estrictas y fuertes]\label{def:funcion_convexa}
...
\begin{definition}[Epígrafe]\label{def:epigrafe}
...
\begin{theorem}[El epígrafe caracteriza la convexidad]\label{thm:epigrafe_convexo}
...
\begin{theorem}[Teorema de minimización convexo]\label{thm:minimizacion_convexa}
...
Es fácil ver que las afirmaciones del \cref{thm:minimizacion_convexa} son verdaderas si sustituimos la minimización de una función convexa por la maximización de una función cóncava.
%%% Local Variables:
%%% mode: LaTeX
