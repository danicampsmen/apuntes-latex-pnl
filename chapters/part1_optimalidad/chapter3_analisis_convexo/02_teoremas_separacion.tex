...
\begin{proposition}\label{v1:prop:3.2.1}
...
\begin{corollary}\label{v1:cor:3.2.1}
 Un conjunto poliédrico en $\Rn$ es convexo.
\end{corollary}

\begin{proof}
 Un conjunto poliédrico es el conjunto de soluciones de un sistema finito de ecuaciones e inecuaciones lineales, es decir, una intersección de semiespacios e hiperplanos que son conjuntos convexos. Por lo tanto, un conjunto poliédrico es convexo por la \cref{v1:prop:3.2.1}.
\end{proof}

A continuación mostramos que la clausura y el interior de un conjunto convexo son conjuntos convexos.

\begin{proposition}\label{v1:prop:3.2.2}
...
\begin{proposition}\label{v1:prop:3.2.3}
...
\begin{definition}[Combinación convexa]\label{v1:def:3.2.1}
 Dados $x^i \in \Rn$, $\alpha_i \in [0, 1]$, $i = 1, \dots, p$, tales que $\sum_{i=1}^p \alpha_i = 1$, el punto $\sum_{i=1}^p \alpha_i x^i$ se llama \textbf{combinación convexa} de los puntos $x^i \in \Rn$ con parámetros $\alpha_i$.
\end{definition}

\begin{theorem}[Caracterización por combinaciones convexas]\label{v1:thm:3.2.1}
...
\begin{proof}
 ...
 \begin{equation}
 x = (1 - \alpha_{j+1})y + \alpha_{j+1} x^{j+1}, \label{v1:eq:3.4}
 \end{equation}
 donde $y = \sum_{i=1}^j \frac{\alpha_i}{1 - \alpha_{j+1}} x^i$. Como los coeficientes de $y$ suman $1$, $y \in D$ por hipótesis de inducción. Luego, \eqref{v1:eq:3.4} muestra que $x$ es una combinación convexa de dos puntos de $D$, por lo que $x \in D$.
\end{proof}

\begin{corollary}[Desigualdad de Jensen]\label{v1:cor:3.2.2}
 ...
 \begin{equation}
 f\left(\sum_{i=1}^p \alpha_i x^i\right) \leq \sum_{i=1}^p \alpha_i f(x^i). \label{v1:eq:3.5}
 \end{equation}
\end{corollary}

\begin{proof}
 Por la definición de epígrafe, $(x^i, f(x^i)) \in E_f$. Por la convexidad de $f$, $E_f$ es un conjunto convexo en $\Rn \times \R$ (\cref{v1:thm:3.1.4}). Por el \cref{v1:thm:3.2.1}, la combinación convexa pertenece a $E_f$:
 \[
 \left(\sum_{i=1}^p \alpha_i x^i, \sum_{i=1}^p \alpha_i f(x^i)\right) = \sum_{i=1}^p \alpha_i (x^i, f(x^i)) \in E_f.
 \]
 Usando la definición de epígrafe, obtenemos la \eqref{v1:eq:3.5}.
\end{proof}

\begin{theorem}[Teorema de Carathéodory]\label{v1:thm:3.2.2}
...
\begin{definition}[Cierre convexo o envoltura convexa]\label{v1:def:3.2.2}
 Sea $D \subset \Rn$ un conjunto cualquiera. El \textbf{cierre convexo} de $D$, denotado por $\conv D$, es el menor conjunto convexo en $\Rn$ que contiene a $D$ (o la intersección de todos los conjuntos convexos que contienen a $D$).
\end{definition}

\begin{proposition}\label{v1:prop:3.2.4}
 El cierre convexo $\conv D$ es el conjunto de todas las combinaciones convexas de puntos de $D$.
\end{proposition}

\begin{proof}
 Sea $C$ el conjunto de todas las combinaciones convexas. Como $\conv D$ es convexo y contiene a $D$, por el \cref{v1:thm:3.2.1} debe contener a $C$, luego $C \subset \conv D$. Por otro lado, es fácil demostrar que $C$ es convexo (la combinación convexa de dos combinaciones convexas sigue siendo una combinación convexa cuyos coeficientes suman 1). Como $D \subset C$ y $C$ es convexo, $\conv D \subset C$.
\end{proof}

\begin{proposition}\label{v1:prop:3.2.5}
 Sea $D \subset \Rn$ un conjunto compacto. Entonces $\conv D$ es compacto.
\end{proposition}

\begin{proof}
 Primero mostraremos que $\conv D$ es acotado. Todo $x \in \conv D$ es una combinación convexa $\sum \alpha_i x^i$. Por la desigualdad triangular, $\norm{x} \leq \sum \alpha_i \norm{x^i} \leq \sup_{y\in D} \norm{y} < +\infty$.
 
 Para ver que es cerrado, sea $\{y^k\} \to y$ con $y^k \in \conv D$. Por el \cref{v1:thm:3.2.2}, $y^k = \sum_{i=1}^{n+1} \alpha_{k,i} x^{k,i}$. Por compacidad, las secuencias $\{x^{k,i}\}$ y $\{\alpha_{k,i}\}$ son acotadas. Pasando a una subsecuencia convergente, los límites $x^i \in D$ y $\alpha_i \ge 0$ satisfacen $\sum \alpha_i = 1$ y $y = \sum \alpha_i x^i \in \conv D$.
\end{proof}

\begin{proposition}\label{v1:prop:3.2.6}
...
\begin{proof}
 La primera igualdad ya fue establecida en la \cref{v1:prop:3.1.3}. $\conv K$ es un cono. De la inclusión $K \subset \conv K$ obtenemos $(\conv K)^* \subset K^*$. Si $y \in K^*$, para cualquier $d = \sum \alpha_i d^i \in \conv K$ con $d^i \in K$, se tiene $\interno{y}{d} = \sum \alpha_i \interno{y}{d^i} \leq 0$. Luego $K^* \subset (\conv K)^* $ .
\end{proof}
...
\begin{definition}[Dirección de recesión]\label{v1:def:3.2.3}
 Decimos que $d \in \Rn$ es una \textbf{dirección de recesión} del conjunto convexo $D \subset \Rn$ cuando:
 \[
...
\begin{proposition}[Cono de recesión de un conjunto convexo]\label{v1:prop:3.2.7}
...
\begin{definition}[Cono con número finito de generadores]\label{v1:def:3.2.4}
 Un cono $K \subset \Rn$ tiene un \textbf{número finito de generadores} cuando existen $a^1, \dots, a^p \in \Rn$ tales que $K = \text{cone}\{a^i, \, i = 1, \dots, p\}$.
\end{definition}

\begin{proposition}\label{v1:prop:3.2.8}
 Sean $a^i \in \Rn$, $i = 1, \dots, p$. Se tiene que:
 \begin{equation}
 \text{cone}\{a^i, \, i = 1, \dots, p\} = \left\{ x \in \Rn \;\middle|\; x = \sum_{i=1}^p \mu_i a^i, \, \mu_i \geq 0, \, i = 1, \dots, p \right\}. \label{v1:eq:3.6}
 \end{equation}
\end{proposition}

\begin{proof}
 Sea $C$ el lado derecho de \eqref{v1:eq:3.6}. $C$ es un cono convexo y contiene los $a^i$, por lo que $\text{cone}\{a^i\} \subset C$. Por otro lado, cualquier $x \in C$ con $\sum \mu_i > 0$ se puede escribir como $(\sum \mu_i) \sum \beta_i a^i$, donde $\beta_i = \mu_i / \sum \mu_i$ suman 1. Así, $x$ es un múltiplo positivo de una combinación convexa de los $a^i$, por lo que $x \in \text{cone}(\conv\{a^i\}) = \text{cone}\{a^i\}$. Luego $C \subset \text{cone}\{a^i\}$.
\end{proof}

\begin{lemma}\label{v1:lem:3.2.1}
...
\begin{definition}[Cono poliédrico]\label{v1:def:3.2.5}
 Decimos que un cono $K \subset \Rn$ es \textbf{poliédrico} si es el conjunto de soluciones de un sistema lineal homogéneo: $K = \{ x \in \Rn \mid Q_1 x \leq 0, \, Q_2 x = 0 \}$.
\end{definition}

\begin{theorem}[Teorema de Minkowski-Weyl]\label{v1:thm:3.2.3}
...
\begin{theorem}[Teorema de la Proyección]\label{v1:thm:3.2.4}
 ...
 \begin{equation}
 \bar{x} \in D, \quad \interno{x - \bar{x}}{y - \bar{x}} \leq 0 \quad \forall y \in D, \label{v1:eq:3.7}
 \end{equation}
...
\begin{corollary}[Operador de proyección no expansivo]\label{v1:cor:3.2.3}
 Sea $D \subset \Rn$ un conjunto convexo y cerrado. Entonces $\forall x, y \in \Rn$:
 \begin{equation}
 \norm{P_D(x) - P_D(y)} \leq \norm{x - y}. \label{v1:eq:3.8}
 \end{equation}
\end{corollary}

\begin{proof}
 Evaluando \eqref{v1:eq:3.7} dos veces y sumando, se obtiene $\norm{P_D(x) - P_D(y)}^2 \le \interno{x - y}{P_D(x) - P_D(y)}$. Aplicando Cauchy-Schwarz, se deduce \eqref{v1:eq:3.8}.
\end{proof}
...
\begin{definition}[Hiperplano de separación]\label{v1:def:3.2.6}
 El hiperplano $H(a, c) = \{ x \in \Rn \mid \interno{a}{x} = c \}$ \textbf{separa} los conjuntos $D_1$ y $D_2$ si:
 \[
 \interno{a}{x^1} \leq c \leq \interno{a}{x^2} \quad \forall x^1 \in D_1, \, \forall x^2 \in D_2.
 \]
 La separación es \textbf{estricta} cuando las desigualdades son estrictas.
\end{definition}

\begin{lemma}[Lema de Minkowski]\label{v1:lem:3.2.2}
...
\begin{proof}
 Sea $\bar{x} = P_{\cl D}(x)$. Definimos $a = \bar{x} - x \neq 0$ y $c = \interno{\bar{x} - x}{x}$. Para $y \in D$, por el \cref{v1:thm:3.2.4}, $\interno{a}{y} = \interno{\bar{x} - x}{y} \geq \interno{\bar{x} - x}{\bar{x}} = c + \norm{\bar{x} - x}^2 > c$.
\end{proof}

\begin{lemma}[Separación en la frontera]\label{v1:lem:3.2.3}
...
\begin{proof}
 Tomamos $\{x^k\} \to x$ con $x^k \notin \cl D$. Por el \cref{v1:lem:3.2.2}, existen $a^k$ (normalizados) y $c_k$. Extraemos una subsucesión convergente $\{a^k\} \to a \neq 0$. En el límite, se cumple la separación.
\end{proof}

\begin{theorem}[Teorema de Separación]\label{v1:thm:3.2.5}
...
\begin{proof}
 El conjunto $D = D_2 - D_1$ es convexo y $0 \notin D$. Si $0 \notin \cl D$, aplicamos el \cref{v1:lem:3.2.2}; si $0 \in \text{fr } D$, aplicamos el \cref{v1:lem:3.2.3}. En ambos casos, existe $a \neq 0$ tal que $\interno{a}{x^2 - x^1} \ge 0$. Elegimos $c = (\sup_{D_1}\interno{a}{x^1} + \inf_{D_2}\interno{a}{x^2})/2$.
\end{proof}

\begin{theorem}[Teorema de Separación Estricta]\label{v1:thm:3.2.6}
...
\begin{proposition}\label{v1:prop:3.2.9}
 Sea $K \subset \Rn$ un cono cualquiera. Se tiene:
 \[
 (K^*)^* = \cl \conv K.
 \]
...
\begin{definition}[Punto extremo]\label{v1:def:3.2.7}
 Decimos que $x \in D$ es un \textbf{punto extremo} del conjunto convexo $D$ cuando no puede ser representado por una combinación convexa de dos puntos diferentes de $D$ (es decir, $x = \alpha x^1 + (1-\alpha)x^2 \implies x=x^1=x^2$). Denotamos el conjunto por $E(D)$.
\end{definition}

\begin{lemma}\label{v1:lem:3.2.4}
...
\begin{proposition}\label{v1:prop:3.2.10}
...
\begin{proof}
 Si $D$ contiene una recta que pasa por $x$, $x$ se puede escribir como el punto medio de $x+d$ y $x-d$, por lo que no es extremo. El recíproco se demuestra por inducción en la dimensión $n$, interceptando el conjunto con hiperplanos de soporte y utilizando el \cref{v1:lem:3.2.4}.
\end{proof}

\begin{theorem}[Teorema de Krein-Milman]\label{v1:thm:3.2.7}
...
\begin{proof}
 Por inducción en $n$. Para $n=1$ es obvio. Supongamos válido en $\Rn$. Para $D \subset \R^{n+1}$, si $\bar{x} \in \text{fr } D$, el hiperplano de soporte reduce la dimensión del problema, y por hipótesis inductiva y el \cref{v1:lem:3.2.4}, $\bar{x}$ se forma por extremos de $D$. Si $\bar{x} \in \interior D$, cualquier recta que pase por $\bar{x}$ corta a la frontera en dos puntos (por compacidad); como $\bar{x}$ es combinación convexa de ellos, queda probado.
\end{proof}

\begin{proposition}\label{v1:prop:3.2.11}
...
\begin{corollary}\label{v1:cor:3.2.4}
 El número de vértices de un conjunto poliédrico cualquiera es finito.
\end{corollary}

\begin{theorem}[Representación de conjuntos poliédricos sin rectas]\label{v1:thm:3.2.8}
