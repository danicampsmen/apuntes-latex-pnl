...
\begin{theorem}[Condición necesaria en forma primal]\label{v1:thm:4.1.3}
 Supongamos que $f : \Rn \to \R$ y $g : \Rn \to \R^m$ son diferenciables en el punto $\bar{x} \in \Rn$, y que $h : \Rn \to \R^l$ es una función diferenciable en una vecindad de este punto, con derivada continua en $\bar{x}$.
 
 Si $\bar{x}$ es un minimizador local del problema \eqref{v1:eq:4.1}-\eqref{v1:eq:4.2} y si alguna de las condiciones de regularidad de las restricciones del \cref{v1:thm:4.1.2} es satisfecha (i.e., linealidad \eqref{v1:eq:4.7}, Mangasarian-Fromovitz \eqref{v1:eq:4.8} o Slater \eqref{v1:eq:4.10}), entonces se tiene que:
 \[
 \langle \nabla f(\bar{x}), d \rangle \ge 0 \quad \forall d \in \mathcal{H}(\bar{x}),
 \]
 donde el cono de linealización de las restricciones activas $\mathcal{H}(\bar{x})$ viene dado por \eqref{v1:eq:4.6}.
\end{theorem}

\begin{theorem}[Teorema de Robinson]\label{v1:thm:4.1.4}
 Sean $h : \Rn \to \R^l$ y $g : \Rn \to \R^m$ funciones diferenciables en una vecindad del punto $\bar{x} \in D$,
 \[
 D = \{x \in \Rn \mid h(x) = 0, \: g(x) \le 0\},
 \]
 con derivadas continuas en este punto. Supongamos que $\bar{x}$ satisface la condición de regularidad de Mangasarian-Fromovitz \eqref{v1:eq:4.8}.
 ...
 \label{v1:eq:4.15}
 \end{equation}
 donde $\Psi : \Rn \to \R^l \times \R^m$ está dada por $\Psi(x) = (h(x), \max\{0, g_1(x)\}, \dots, \max\{0, g_m(x)\})$.
\end{theorem}
...
En esta sección deducimos las condiciones analíticas de optimalidad de primer orden para problemas con restricciones mixtas a partir de la caracterización del cono tangente desarrollada en la sección anterior. Como se demostró, la condición necesaria primal (\cref{v1:thm:4.1.3}) establece que el opuesto del gradiente pertenece al cono dual del cono de linealización: $-\nabla f(\bar{x}) \in \mathcal{H}(\bar{x})^*$. Utilizando la parametrización de este cono dual obtenida en el \cref{v1:lem:dual_cono_linealizacion}, expresaremos esta relación geométrica como un sistema algebraico de multiplicadores de Lagrange, conocido como condiciones de Karush-Kuhn-Tucker (KKT).

\begin{theorem}[Condiciones de Karush-Kuhn-Tucker]\label{v1:thm:4.2.1}
 Supongamos que $\bar{x} \in D$ es una solución local del problema \eqref{v1:eq:4.1}-\eqref{v1:eq:4.2}, donde $f, h, g$ son funciones continuamente diferenciables en $\bar{x}$.
 
 Si el punto $\bar{x}$ satisface alguna de las condiciones de regularidad de las restricciones detalladas en la \cref{v1:def:regularidad_mixta} (independencia lineal, linealidad afín, Slater o MFCQ), entonces existen multiplicadores $\bar{\lambda} \in \R^l$ y $\bar{\mu} \in \R^m$ con $\bar{\mu} \ge 0$, tales que:
 \begin{align}
 \nabla f(\bar{x}) + \sum_{i=1}^l \bar{\lambda}_i \nabla h_i(\bar{x}) + \sum_{i=1}^m \bar{\mu}_i \nabla g_i(\bar{x}) &= 0, \label{v1:eq:4.18} \\
 \bar{\mu}_i g_i(\bar{x}) &= 0 \quad \forall i \in \{1,\dots,m\}. \label{v1:eq:4.19}
 \end{align}
\end{theorem}

\begin{proof}
 Bajo cualquiera de las condiciones de regularidad supuestas, el Teorema de la Condición Necesaria Primal (\cref{v1:thm:4.1.3}) establece que la derivada direccional es no negativa sobre todo el cono de linealización, es decir, $\langle \nabla f(\bar{x}), d \rangle \ge 0 \quad \forall d \in \mathcal{H}(\bar{x})$. En términos de conos duales, esto equivale a la afirmación geométrica:
 \[
 -\nabla f(\bar{x}) \in \mathcal{H}(\bar{x})^*.
 \]
 Por el \cref{v1:lem:dual_cono_linealizacion}, conocemos la parametrización de los elementos de $\mathcal{H}(\bar{x})^*$. Por lo tanto, se garantiza la existencia de multiplicadores $\bar{\lambda} \in \R^l$ y $\hat{\mu}_i \ge 0$ para $i \in I(\bar{x})$ tales que:
 \[
 -\nabla f(\bar{x}) = \sum_{i=1}^l \bar{\lambda}_i \nabla h_i(\bar{x}) + \sum_{i \in I(\bar{x})} \hat{\mu}_i \nabla g_i(\bar{x}).
 \]
 Definimos el vector $\bar{\mu} \in \R^m$ haciendo $\bar{\mu}_i = \hat{\mu}_i$ si $i \in I(\bar{x})$, y $\bar{\mu}_i = 0$ si $i \notin I(\bar{x})$. Al agrupar los términos se obtiene la condición de estacionariedad \eqref{v1:eq:4.18}.
 
 Por otra parte, si la restricción $i$ es activa ($i \in I(\bar{x})$), se tiene $g_i(\bar{x}) = 0$, lo que anula el producto $\bar{\mu}_i g_i(\bar{x})$. Si es inactiva ($i \notin I(\bar{x})$), por construcción se tiene $\bar{\mu}_i = 0$, lo que también anula el producto. Esto demuestra la condición de complementariedad \eqref{v1:eq:4.19}.
\end{proof}
...
\begin{definition}[Complementariedad Estricta]\label{v1:def:complementariedad_estricta}
...
\begin{example}[Análisis por casos del sistema KKT]\label{v1:exa:kkt_mixto_esfera}
...
\begin{definition}[Sistema KKT y Lagrangiana]\label{v1:def:kkt_sistema}
...
\begin{theorem}[Suficiencia de KKT bajo Convexidad]\label{v1:thm:4.2.2}
 Sea $\bar{x} \in \Rn$ un punto estacionario del problema \eqref{v1:eq:4.1}-\eqref{v1:eq:4.2} (es decir, $\mathcal{M}(\bar{x}) \neq \emptyset$).
...
\begin{notabox}[Dualidad y condiciones de punto de silla]
 La demostración del \cref{v1:thm:4.2.2} utiliza la propiedad de punto de silla de la función Lagrangiana:
 \[
...
\begin{example}[Falta de convexidad en las restricciones]\label{v1:exa:falla_kkt_convexo}
 ...
 No es posible aplicar el \cref{v1:thm:4.2.2} en este caso debido a que la función $g(x) = -x_1^2 + x_2$ no es convexa en $\R^2$ (su Hessiano es indefinido). El teorema exige la convexidad de las funciones que definen las restricciones, no únicamente la del conjunto factible resultante.
\end{example}

\begin{example}[Resolución de un problema convexo]\label{v1:exa:4.2.1}
...
\begin{definition}[Condición Estricta de Mangasarian-Fromovitz - SMFCQ]\label{v1:def:smfcq}
...
\begin{proposition}[Propiedades del Conjunto de Multiplicadores]\label{v1:prop:4.2.2}
...
\begin{proof}
 ...
 \]
 Por la caracterización dual de Mangasarian-Fromovitz (\cref{v1:prop:4.1.2}), si se cumple MFCQ este sistema homogéneo admite únicamente la solución trivial $(\tilde{\lambda}, \tilde{\mu}) = 0$. Esto contradice que el vector límite tiene norma unitaria, concluyendo que el conjunto de multiplicadores es compacto.
\end{proof}

\begin{example}[Geometría del conjunto de multiplicadores]\label{v1:exa:propiedades_multiplicadores}
...
\begin{theorem}[Condiciones de Fritz John]\label{v1:thm:4.2.3}
 Sea $\bar{x} \in D$ un minimizador local de \eqref{v1:eq:4.1}-\eqref{v1:eq:4.2}. Independientemente de las condiciones de regularidad, existe un vector no nulo $(\lambda_0, \bar{\lambda}, \bar{\mu}) \in \R \times \R^l \times \R^m$ tal que:
 \begin{align}
 \lambda_0 \nabla f(\bar{x}) + \sum_{i=1}^l \bar{\lambda}_i \nabla h_i(\bar{x}) + \sum_{i=1}^m \bar{\mu}_i \nabla g_i(\bar{x}) &= 0, \\
 \bar{\mu}_i g_i(\bar{x}) &= 0 \quad \forall i=1,\dots,m, \nonumber \\
...
\begin{proof}
 ...
 \textbf{Caso 1:} Si se cumple la condición de regularidad MFCQ, por el \cref{v1:thm:4.2.1} se garantiza la existencia de multiplicadores KKT $(\bar{\lambda}, \bar{\mu})$. En este caso basta definir $\lambda_0 = 1$ para obtener la solución $(1, \bar{\lambda}, \bar{\mu}) \neq 0$.
 
 \textbf{Caso 2:} Si no se cumple MFCQ, por la \cref{v1:prop:4.1.2} existe una solución no nula del sistema dual homogéneo:
 \[
...
\begin{example}[Puntos estacionarios con $\lambda_0 = 0$]\label{v1:exa:fritz_john_cuspide}
 Considérese el problema:
