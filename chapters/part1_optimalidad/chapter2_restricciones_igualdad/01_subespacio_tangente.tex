...
\chapter{Restricciones de igualdad}
\label{v1:chap:2}
...
\begin{definition}[Problema con restricciones de igualdad]\label{v1:def:2.1.1}
...
\begin{proposition}\label{v1:prop:2.1.1}
...
\begin{definition}[Condiciones de regularidad]\label{v1:def:2.1.2}
...
\begin{lemma}[Existencia de Función Implícita Acotada]\label{v1:lem:2.1.1}
...
\begin{example}\label{v1:exa:2.1.1}
 ...
 \]
 El determinante de esta matriz es $-2 \neq 0$. Por lo tanto, $\rk J_y F(\bar{p}, \bar{y}) = 2 = l$, por lo que las hipótesis del \cref{v1:lem:2.1.1} se cumplen.
...
\begin{theorem}[Lyusternik: Estimación de Viabilidad]\label{v1:thm:2.1.1}
...
\begin{proof}
 Definimos el campo paramétrico $F : \Rn \times \Rn \to \R^l$ mediante la traslación $F(x, \xi) = h(x + \xi)$. Tratando a $x$ como el parámetro y a $\xi$ como la variable, evaluamos en el punto base $(\bar{x}, 0)$. Se tiene $F(\bar{x}, 0) = h(\bar{x}) = 0$. El Jacobiano respecto a la variable $\xi$ es $J_\xi F(\bar{x}, 0) = J_h(\bar{x})$, el cual, por la hipótesis LICQ, tiene rango máximo $l$. Las hipótesis del \cref{v1:lem:2.1.1} se verifican.
 
 Por consiguiente, existen una vecindad $U$ de $\bar{x}$ y un número $M > 0$ tales que, para cada $x \in U$, existe una perturbación $\xi(x) \in \Rn$ que cumple $0 = F(x, \xi(x)) = h(x + \xi(x))$. Esto asegura que el punto trasladado $x + \xi(x) \in D$. Además, el \cref{v1:lem:2.1.1} establece la cota $\norm{\xi(x)} \le M \norm{F(x, 0)} = M \norm{h(x)}$.
...
\begin{example}[Acotación de Lyusternik en la Esfera]\label{v1:exa:2.1.2}
 Considérese la variedad esférica unitaria en $\R^2$, definida por $h(x) = x_1^2 + x_2^2 - 1 = 0 \implies D = \{ x \in \R^2 \mid \norm{x}^2 = 1 \}$.
 Tomemos un punto nominal en el polo norte, $\bar{x} = (0,1) \in D$. El Jacobiano evaluado en este punto es $J_h(\bar{x}) = (0, 2)$. Dado que $\rk J_h(\bar{x}) = 1$, la condición LICQ se cumple en $\bar{x}$. El \cref{v1:thm:2.1.1} asegura la existencia de una constante $M > 0$ y una vecindad $U$ de $\bar{x}$ tal que $\dist(x, D) \le M \absolute{h(x)}$ para todo $x \in U$.
...
\begin{theorem}[Teorema del Subespacio Tangente]\label{v1:thm:2.1.2}
...
\begin{proof}
 \textbf{Parte (1):} Por la \cref{v1:prop:2.1.1}, $\mathcal{B}_D(\bar{x}) \subset \ker J_h(\bar{x})$. Inversamente, sea $d \in \ker J_h(\bar{x})$. Por el Teorema de Lyusternik (\cref{v1:thm:2.1.1}), para $t > 0$ suficientemente pequeño se tiene $\dist(\bar{x} + td, D) \le M \norm{h(\bar{x} + td)}$. Expandiendo $h$ por Taylor y utilizando que $h(\bar{x}) = 0$ y $J_h(\bar{x})d = 0$:
	\[
...
\begin{example}[Colapso Analítico del Cono Tangente en $\R^3$]\label{v1:exa:2.1.3}
...
\begin{theorem}[Condición necesaria en forma primal]\label{v1:thm:2.1.3}
 Sea $\bar{x} \in D$ un minimizador local del problema \eqref{v1:eq:2.1}. Si $\bar{x}$ es regular (cumple LICQ o linealidad), entonces el gradiente de la función objetivo se anula a lo largo de todo el subespacio tangente:
 \begin{equation}
...
\begin{proof}
 De acuerdo con la caracterización general del \cref{v1:chap:1}, si $\bar{x}$ es un mínimo local, se cumple que $\interno{\nabla f(\bar{x})}{d} \ge 0$ para todo $d \in \mathcal{B}_D(\bar{x})$. Por la regularidad y el \cref{v1:thm:2.1.2}, se tiene $\mathcal{B}_D(\bar{x}) = \ker J_h(\bar{x})$. Esta desigualdad se aplica sobre el subespacio nulo. Dado que $\ker J_h(\bar{x})$ es un subespacio vectorial, si $d \in \ker J_h(\bar{x})$, su opuesto simétrico $-d$ también pertenece a $\ker J_h(\bar{x})$. Evaluando la desigualdad en dicho vector: $\interno{\nabla f(\bar{x})}{-d} \ge 0 \implies \interno{\nabla f(\bar{x})}{d} \le 0$. De ambas desigualdades se deduce la igualdad a cero.
\end{proof}

\begin{example}[Ortogonalidad Primal en el Cilindro]\label{v1:exa:2.1.4}
...
\begin{corollary}[Singularidad Topológica por Exceso de Restricciones]\label{v1:cor:2.1.1}
