...
\begin{mini}{x}{f(x)}{\label{v1:eq:primal_general}}{}
...
\begin{definition}[Función Lagrangiana]\label{v1:def:lagrangiana_general}
    Asociada al problema primal \eqref{v1:eq:primal_general}, se define la \textbf{función Lagrangiana} con respecto a las restricciones funcionales como la aplicación $L \colon \Omega \times \R^l \times \R^m \to \R$ dada por:
    \begin{equation}
...
\begin{lemma}[Penalización implícita]\label{v1:lem:sup_lagrangiana}
...
\begin{remark}
    El \cref{v1:lem:sup_lagrangiana} permite reescribir de forma equivalente el problema primal \eqref{v1:eq:primal_general} como un problema (tipo minimax) sin restricciones funcionales, sobre el conjunto base $\Omega$:
    \begin{equation}
        \min \left\{ \sup_{(\lambda, \mu) \in \R^l \times \R^m_+} L(x, \lambda, \mu) \right\} \quad \text{sujeto a} \quad x \in \Omega.
        \label{v1:eq:minimax_primal_problem}
    \end{equation}
    ...
    \begin{equation}
        ...
        \label{v1:eq:minimax_primal_value}
    \end{equation}
...
\begin{example}[Evaluación del supremo de la Lagrangiana]\label{v1:exa:penalizacion_lagrangiana}
...
El \textbf{problema dual} se define alternando el orden de los operadores de optimización en la formulación minimax del primal \eqref{v1:eq:minimax_primal_problem}:
\begin{equation}
    \bar{w} = \max_{\substack{\lambda \in \R^l \\ \mu \ge 0}} \left\{ \inf_{x \in \Omega} L(x, \lambda, \mu) \right\}.
    \label{v1:eq:minimax_dual}
\end{equation}
...
\begin{equation}
    ...
    \label{v1:eq:dual_general}
\end{equation}

\begin{theorem}[Propiedades de la función dual]\label{v1:thm:propiedades_dual}
...
\begin{theorem}[Teorema de Dualidad Débil]\label{v1:thm:dualidad_debil_general}
...
\begin{example}[Obtención analítica de la función dual]\label{v1:exa:propiedades_dual}
...
\begin{corollary}[Ausencia de salto de dualidad]\label{v1:cor:no_gap}
...
\begin{example}[Mínimo primal no alcanzado, sin salto de dualidad]\label{v1:exa:no_primal_sol}
...
\begin{example}[Óptimo primal existente, sin salto, pero dual no soluble]\label{v1:exa:no_dual_sol}
...
\begin{example}[Salto de dualidad estrictamente positivo]\label{v1:exa:duality_gap}
...
\begin{definition}[Multiplicador de Lagrange Global]\label{v1:def:lagrange_multiplier_global}
    Un vector $(\bar{\lambda}, \bar{\mu}) \in \R^l \times \R^m_+$ es un \textbf{multiplicador de Lagrange global} para el problema primal \eqref{v1:eq:primal_general} si cumple:
    \begin{equation}
        \bar{v} = \inf_{x \in \Omega} L(x, \bar{\lambda}, \bar{\mu}).
        \label{v1:eq:def_multiplier}
    \end{equation}
\end{definition}
...
\begin{proposition}[Identidad entre Multiplicadores y Soluciones Duales]\label{v1:prop:multipliers_dual_solutions}
...
\begin{example}[Identidad en un problema cuadrático bidimensional]\label{v1:exa:identidad_multiplicadores}
    ...
    Observamos empíricamente que $\bar{v} = \bar{w} = -2$ (brecha nula). El óptimo dual $\bar{\mu} = 1/2$ debe satisfacer la definición topológica de un multiplicador global (\cref{v1:def:lagrange_multiplier_global}). Comprobémoslo:
    \[
...
\begin{theorem}[Garantía de Existencia de Solución Dual]\label{v1:thm:dual_solvability_multiplier}
...
\begin{proof}
    Sea $(\bar{\lambda}, \bar{\mu}) \in \R^l \times \R^m_+$ un multiplicador de Lagrange. Por definición (\cref{v1:def:lagrange_multiplier_global}), $\varphi(\bar{\lambda}, \bar{\mu}) = \bar{v}$.
    
    Si $\Delta \neq \emptyset$, por el Teorema de Dualidad Débil (\cref{v1:thm:dualidad_debil_general}), $\bar{v}$ acota superiormente a cualquier valor factible dual. Puesto que el par $(\bar{\lambda}, \bar{\mu})$ alcanza este límite superior, pertenece a $\Delta$ y es un maximizador global de la función dual.
...
\begin{example}[Infactibilidad dual y divergencia primal]\label{v1:exa:teorema_existencia_dual}
    ...
    Construyamos la función Lagrangiana para $\mu \ge 0$: $L(x, \mu) = x_1(1 + \mu) + x_2(\mu - 1)$. Para que el ínfimo sea finito, se requeriría $1 + \mu = 0$ y $\mu - 1 = 0$, lo cual es una contradicción. Concluimos que para cualquier $\mu \ge 0$, $\varphi(\mu) = -\infty$. Por tanto, $\Delta = \emptyset$, lo que corrobora el Teorema \ref{v1:thm:dual_solvability_multiplier}.
\end{example}

\begin{theorem}[Condiciones de Kuhn-Tucker No Diferenciables]\label{v1:thm:kuhn_tucker_non_diff}
    ...
    \begin{align}
        L(\bar{x}, \bar{\lambda}, \bar{\mu}) & = \min_{x \in \Omega} L(x, \bar{\lambda}, \bar{\mu}), \label{v1:eq:kt_min_global} \\
        \bar{\mu}_i g_i(\bar{x})             & = 0, \quad \forall i = 1, \dots, m. \label{v1:eq:kt_complementarity_non_diff}
    \end{align}
\end{theorem}

\begin{proof}
    \( (\implies) \) Como $\bar{x}$ es óptimo y $(\bar{\lambda}, \bar{\mu})$ es multiplicador, se tiene $f(\bar{x}) = \inf_{x \in \Omega} L(x, \bar{\lambda}, \bar{\mu})$. Como $\bar{x} \in D$, resulta $L(\bar{x}, \bar{\lambda}, \bar{\mu}) = f(\bar{x}) + \interno{\bar{\mu}}{g(\bar{x})}$. La condición $\bar{\mu} \ge 0, g(\bar{x}) \le 0$ exige que $\interno{\bar{\mu}}{g(\bar{x})} \le 0$. Al ser el ínfimo $f(\bar{x})$, se debe verificar de forma obligatoria que $L(\bar{x}, \bar{\lambda}, \bar{\mu}) = f(\bar{x})$, lo que prueba \eqref{v1:eq:kt_min_global} y requiere $\bar{\mu}_i g_i(\bar{x}) = 0$.
    
    \( (\impliedby) \) Por holgura complementaria y factibilidad, se tiene $L(\bar{x}, \bar{\lambda}, \bar{\mu}) = f(\bar{x})$. Sustituyendo en la minimización global \eqref{v1:eq:kt_min_global}, resulta $f(\bar{x}) = \inf_{x \in \Omega} L(x, \bar{\lambda}, \bar{\mu})$, lo cual demuestra que $(\bar{\lambda}, \bar{\mu})$ es un multiplicador y $\bar{x}$ es óptimo global.
\end{proof}
...
\begin{definition}[Condición de Regularidad de Slater]\label{v1:def:slater}
    Se dice que el problema primal \eqref{v1:eq:primal_general} satisface la \textbf{condición de Slater} si las restricciones de igualdad son afines ($h(x) = Ax - a$), las restricciones de desigualdad $g_i$ son funciones convexas en $\Omega$, y existe un punto de regularidad $\hat{x} \in \interior \Omega$ tal que:
    \begin{equation}
        A\hat{x} - a = 0 \quad \text{y} \quad g_i(\hat{x}) < 0 \quad \forall i = 1, \dots, m.
    \end{equation}
\end{definition}

\begin{theorem}[Teorema de Dualidad Fuerte de Slater]\label{v1:thm:dualidad_fuerte_slater}
    Supóngase que en el problema primal \eqref{v1:eq:primal_general} las funciones $f$ y $g_i$ son convexas, las restricciones de igualdad son afines dadas por $h(x) = Ax - a$ con $A \in \R^{l \times n}$, y el valor óptimo primal $\bar{v}$ es finito. Si se satisface la condición de regularidad de Slater (\cref{v1:def:slater}), entonces el salto de dualidad es nulo ($\bar{v} = \bar{w}$), y el problema dual admite al menos una solución óptima $(\bar{\lambda}, \bar{\mu}) \in \Delta$.
\end{theorem}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \label{v1:eq:slater_separacion}
    \end{equation}
    Para que la desigualdad se cumpla asintóticamente es necesario que $\beta \ge 0$ y $\bar{\mu} \ge 0$.
    
    Asumamos que $\beta = 0$. Evaluando \eqref{v1:eq:slater_separacion} en el punto factible de Slater $\hat{x}$, se obtiene $0 \le \interno{\bar{\mu}}{g(\hat{x})}$. Dado que $\bar{\mu} \ge 0$ y cada componente $g_i(\hat{x}) < 0$, la única posibilidad es que $\bar{\mu} = 0$. Sustituyendo $\beta = 0$ y $\bar{\mu} = 0$, nos queda $0 \le \interno{\bar{\lambda}}{c}$ para todo $c$ factible. Como $A$ tiene rango completo, esto es posible solo si $\bar{\lambda} = 0$. Esto implica que el vector normal es nulo, contradiciendo el Teorema de Separación. Por consiguiente, $\beta > 0$.
    
    Dividiendo \eqref{v1:eq:slater_separacion} por $\beta$ y definiendo $\lambda^* = \bar{\lambda}/\beta \in \R^l$ y $\mu^* = \bar{\mu}/\beta \in \R^m_+$. Tomando $(y, z, c) = (f(x), g(x), Ax - a) \in C$, la inecuación se reduce a:
    \[
        \bar{v} \le f(x) + \interno{\mu^*}{g(x)} + \interno{\lambda^*}{Ax - a} = L(x, \lambda^*, \mu^*).
    \]
    Tomando el ínfimo sobre todo $x \in \Omega$, concluimos que $\bar{v} \le \varphi(\lambda^*, \mu^*)$. A su vez, por Dualidad Débil (\cref{v1:thm:dualidad_debil_general}), sabemos que $\varphi(\lambda^*, \mu^*) \le \bar{v}$. Así, $\varphi(\lambda^*, \mu^*) = \bar{v}$, probando que no hay salto de dualidad y el óptimo dual se alcanza.
\end{proof}

\begin{example}[Verificación de la dualidad fuerte]\label{v1:exa:dualidad_fuerte_slater}
...
\begin{definition}[Problema Dual de Wolfe]\label{v1:def:wolfe_dual}
    Asociado al problema primal \eqref{v1:eq:primal_general}, asumiendo que $\Omega$ es abierto en $\Rn$ y que $f, g, h$ son continuamente diferenciables, el \textbf{problema dual de Wolfe} se define como:
    \begin{equation}
        \max_{\substack{x \in \Omega, \lambda \in \R^l \\ \mu \ge 0}} L(x, \lambda, \mu) \quad \text{sujeto a} \quad \nabla_x L(x, \lambda, \mu) = 0.
        \label{v1:eq:wolfe_dual}
    \end{equation}
    Denotaremos al conjunto de factibilidad de este problema como $W$.
\end{definition}

\begin{theorem}[Dualidad Débil de Wolfe]\label{v1:thm:dualidad_debil_wolfe}
...
\begin{definition}[Punto de Silla]\label{v1:def:punto_silla}
    ...
    \begin{equation}
        ...
        \label{v1:eq:punto_silla}
    \end{equation}
    para todo $x \in \Omega$, todo $\lambda \in \R^l$ y todo $\mu \ge 0$.
\end{definition}

\begin{theorem}[Teorema del Punto de Silla]\label{v1:thm:teorema_punto_silla}
...
\begin{proposition}[Subgradiente de la Función Dual]\label{v1:prop:dual_subgradient}
...
\begin{controlbox}[Interpretación de puntos de silla]
    Si se encuentra un punto de silla para la función Lagrangiana de un problema no convexo, ¿se puede asegurar que dicho punto define la solución global del problema primal? Justifique su respuesta basándose en las hipótesis del \cref{v1:thm:teorema_punto_silla}.
\end{controlbox}
