...
\begin{proposition}[Teorema de Dualidad Débil]\label{prop:dualidad_debil}
    Para cualesquiera puntos factibles $x \in D$ y $\mu \in \Delta$, se cumple:
    \begin{equation}
        \interno{c}{x} \ge \interno{b}{\mu}.
        \label{eq:dualidad_debil}
    \end{equation}
\end{proposition}
...
\begin{notabox}[Interpretación de la Dualidad Débil]
    Si encontramos $\bar{x} \in D$ y $\bar{\mu} \in \Delta$ tales que $\interno{c}{\bar{x}} = \interno{b}{\bar{\mu}}$, la desigualdad \eqref{eq:dualidad_debil} fuerza a que $\bar{x}$ sea un mínimo global del primal y $\bar{\mu}$ un máximo global del dual.
\end{notabox}

\begin{theorem}[Caracterización de Optimalidad]\label{thm:optimalidad_lp}
    Un punto $\bar{x}$ es una solución del problema primal \eqref{eq:primal_lp} si, y solo si, existe un punto $\bar{\mu}$ que es factible para el problema dual \eqref{eq:dual_lp} y tal que:
    \begin{equation}
        \interno{c}{\bar{x}} = \interno{b}{\bar{\mu}},
    \end{equation}
    es decir, los valores de las funciones objetivo de ambos problemas coinciden.
    
    Más aún, la última afirmación es equivalente a decir que $\bar{\mu}$ es una solución óptima del problema dual \eqref{eq:dual_lp}, o que $\bar{\mu}$ es un multiplicador de Lagrange asociado a la solución $\bar{x}$ del problema primal.
\end{theorem}

\begin{proof}
    Por las condiciones necesarias y suficientes de optimalidad de primer orden para problemas afines, $\bar{x}$ es una solución de \eqref{eq:primal_lp} si, y solo si, existe $\bar{\mu} \in \R^m_+$ tal que:
    \[
    ...
    \]
    Por último, la relación de dualidad débil de la \cref{prop:dualidad_debil} implica que $\bar{\mu}$ es necesariamente una solución del problema dual \eqref{eq:dual_lp}, ya que la función objetivo dual alcanza en el punto $\bar{\mu}$ su cota superior global dentro del conjunto factible.
\end{proof}

\begin{example}[Demostración del Lema de Farkas vía Dualidad]\label{ex:farkas_dualidad}
    El Lema de Farkas establece que el sistema de desigualdades lineales:
    \begin{equation}
        Ax \le 0, \quad \interno{b}{x} > 0
        \label{eq:farkas_sys1}
    \end{equation}
    carece de solución $x \in \Rn$ si y solo si el sistema alternativo:
    \begin{equation}
        A^\top \mu = b, \quad \mu \ge 0
        \label{eq:farkas_sys2}
    \end{equation}
    ...
    \]
    El origen $x = 0$ siempre es factible y su valor óptimo es nulo. El sistema \eqref{eq:farkas_sys1} no tiene solución si y solo si no existe ningún punto factible con valor objetivo estrictamente menor que cero. Esto equivale a decir que $\bar{x} = 0$ es un mínimo global, con valor óptimo $\bar{v} = 0$.
    
    Por el \cref{thm:optimalidad_lp}, esto ocurre si y solo si el dual:
    \[
        \max \interno{0}{\mu} \quad \text{sujeto a} \quad -A^\top \mu = -b, \quad \mu \ge 0
    \]
    es factible. La restricción de igualdad es exactamente $A^\top \mu = b$. Así, el sistema \eqref{eq:farkas_sys1} es incompatible si y solo si el sistema dual \eqref{eq:farkas_sys2} es compatible.
\end{example}

\begin{example}[Verificación numérica]\label{ex:optimalidad_lp}
    Tomemos el problema de minimización:
       \begin{mini*}{ }{3x_1 + 4x_2}{}{ }
        ...
        \end{mini*}
    En el formato estándar \eqref{eq:primal_lp}, definimos:
    \[
    ...
    \]
    Al cumplirse $\interno{c}{\bar{x}} = \interno{b}{\bar{\mu}} = 37/3$, el \cref{thm:optimalidad_lp} garantiza que $\bar{x}$ y $\bar{\mu}$ son soluciones óptimas globales de sus respectivos problemas.
\end{example}

\begin{theorem}[Teorema de Dualidad en Programación Lineal]\label{thm:dualidad_fuerte_lp}
    Para cualquier par de problemas primal \eqref{eq:primal_lp} y dual \eqref{eq:dual_lp}, se cumple exactamente una de las siguientes afirmaciones:
    \begin{enuthm}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \label{eq:sistema_alternativa}
    \end{equation}
    no tiene solución por la definición de ínfimo. Al aplicar el Teorema de la Alternativa de Motzkin, la incompatibilidad de \eqref{eq:sistema_alternativa} implica que el sistema dual homogéneo asociado:
    \begin{equation}
        B^\top \mu - yc = 0, \quad -\interno{b}{\mu} + y\bar{v} + \eta = 0, \quad \mu \ge 0, \quad y \ge 0, \quad \eta \ge 0
        \label{eq:motzkin_system}
    \end{equation}
...
\begin{example}[Problemas ilimitados e infactibles]\label{ex:dualidad_casosextremos}
