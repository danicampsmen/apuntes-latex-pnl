...
\label{v1:sec:condiciones_primales}
% =========================================================

Consideramos ahora un problema en formato general
\begin{equation}
    \min f(x) \quad \text{sujeto a} \quad x \in D, \label{v1:eq:problema_general_restricciones}
\end{equation}
...
\begin{definition}[Dirección viable]\label{v1:def:direccion_viable}
...
\begin{definition}[Cono]\label{v1:def:cono}
...
\begin{definition}[Dirección de descenso]\label{v1:def:direccion_descenso}
...
\begin{lemma}[Direcciones de descenso]\label{v1:lem:caracterizacion_descenso}
...
Si $\bar{x} \in D$ es un minimizador local de \eqref{v1:eq:problema_general_restricciones}, obviamente $\mathcal{D}_f(\bar{x}) \cap \mathcal{V}_D(\bar{x}) = \emptyset$. Por el \cref{v1:lem:caracterizacion_descenso}, tenemos entonces que $\interno{\nabla f(\bar{x})}{d} \ge 0$ para todo $d \in \mathcal{V}_D(\bar{x})$. Sin embargo, esta condición no siempre proporciona información útil, porque $\mathcal{V}_D(\bar{x})$ puede contener solo al vector nulo. Necesitamos considerar las \textbf{direcciones tangentes}.

\begin{definition}[Dirección tangente y cono tangente]\label{v1:def:cono_tangente}
...
\begin{equation}
    \mathcal{B}_D(\bar{x}) = \left\{ d \in \Rn \;\middle|\; \begin{aligned} &\exists \{t_k\} \subset \R_+, \, \{t_k\} \to 0^+, \\ &\dist(\bar{x} + t_k d, D) = o(t_k) \end{aligned} \right\}. \label{v1:eq:cono_bouligand}
\end{equation}
...
\begin{example}\label{v1:exa:cono_bouligand_tangente}
...
\begin{theorem}[Condición necesaria en forma primal]\label{v1:thm:cn_primal}
    ...
    \begin{equation}
        \interno{\nabla f(\bar{x})}{d} \ge 0 \quad \forall d \in \mathcal{B}_D(\bar{x}). \label{v1:eq:condicion_primal_primer_orden}
    \end{equation}
\end{theorem}
...
\begin{proof}
    ...
    \]
    Dividiendo por $t_k > 0$ y pasando al límite, obtenemos \eqref{v1:eq:condicion_primal_primer_orden}.
\end{proof}

Decimos que $\bar{x} \in \Rn$ es un \textbf{punto estacionario} del problema \eqref{v1:eq:problema_general_restricciones} si $\bar{x} \in D$ y vale \eqref{v1:eq:condicion_primal_primer_orden}. 

\begin{corollary}\label{v1:cor:estacionario_interior}
    Sea $\bar{x} \in \interior D$ una solución local del problema \eqref{v1:eq:problema_general_restricciones} tal que $f$ es diferenciable en $\bar{x}$. Entonces $\nabla f(\bar{x}) = 0$.
\end{corollary}

La condición de optimalidad puede transformarse a la forma primal-dual usando el cono dual.

\begin{definition}[Cono dual]\label{v1:def:cono_dual}
...
Usando esta noción, la condición de optimalidad \eqref{v1:eq:condicion_primal_primer_orden} es equivalente a
\begin{equation}
    -\nabla f(\bar{x}) \in (\mathcal{B}_D(\bar{x}))^*. \label{v1:eq:condicion_dual_primal}
\end{equation}
...
\begin{theorem}[Condición suficiente en forma primal]\label{v1:thm:cs_primal}
    Sean $D \subset \Rn$ y $f : \Rn \to \R$ diferenciable en $\bar{x} \in D$. Entonces
    \begin{equation}
        \interno{\nabla f(\bar{x})}{d} > 0 \quad \forall d \in \mathcal{B}_D(\bar{x}) \setminus \{0\} \label{v1:eq:cs_primal}
    \end{equation}
    si, y solamente si, $f$ satisface en $D$ en torno a $\bar{x}$ la condición de crecimiento lineal:
    \begin{equation}
        f(x) - f(\bar{x}) \ge \beta \norm{x - \bar{x}} \quad \forall x \in D \cap U. \label{v1:eq:condicion_crecimiento_lineal}
    \end{equation}
    En particular, \eqref{v1:eq:condicion_crecimiento_lineal} implica que $\bar{x}$ es minimizador local estricto.
\end{theorem}

\begin{proof}
    Supongamos que \eqref{v1:eq:cs_primal} valga, pero no \eqref{v1:eq:condicion_crecimiento_lineal}. Existe $\{x^k\} \subset D \setminus \{\bar{x}\}$ convergiendo a $\bar{x}$ tal que $f(x^k) - f(\bar{x}) < \frac{1}{k} \norm{x^k - \bar{x}}$. Definiendo $t_k = \norm{x^k - \bar{x}}$ y $d^k = (x^k - \bar{x})/t_k$, podemos extraer una subsucesión convergente $d^k \to d$ ($\norm{d}=1$). Así $d \in \mathcal{B}_D(\bar{x}) \setminus \{0\}$. Evaluando el gradiente, obtenemos $0 \ge \interno{\nabla f(\bar{x})}{d}$, en contradicción con \eqref{v1:eq:cs_primal}.
    
    El recíproco es directo evaluando \eqref{v1:eq:condicion_crecimiento_lineal} a lo largo de las trayectorias que definen el cono de Bouligand y tomando el límite para el gradiente.
\end{proof}

\begin{proposition}[Relaciones entre conos]\label{v1:prop:relaciones_conos}
