...
% =========================================================
\label{v1:chap:intro_opt}
\section{Definiciones y algunos hechos básicos}
\label{v1:sec:hechos_basicos}
% =========================================================
...
\begin{equation}
    ...
    \label{v1:eq:problema_general_opt}
\end{equation}
...
\begin{definition}[Minimizadores]\label{v1:def:minimizadores}
    Dado el problema \eqref{v1:eq:problema_general_opt}, un punto $\bar{x} \in D$ se denomina:
    \begin{enuthm}
        \item \textbf{Minimizador global} de \eqref{v1:eq:problema_general_opt}, si
        \[
            f(\bar{x}) \le f(x) \quad \forall x \in D;
        \]
        \item \textbf{Minimizador local} de \eqref{v1:eq:problema_general_opt}, si existe una vecindad $U$ de $\bar{x}$ tal que
        \[
...
\begin{definition}[Valor óptimo]\label{v1:def:valor_optimo}
    El \textbf{valor óptimo} del problema \eqref{v1:eq:problema_general_opt} es el valor $\bar{v} \in [-\infty, +\infty)$ definido por
    \[
...
\begin{proposition}\label{v1:prop:crecimiento_implica_estricto}
...
\begin{equation}
    \max f(x) \quad \text{sujeto a} \quad x \in D \label{v1:eq:problema_max}
\end{equation}
...
\begin{proposition}[Equivalencia entre maximización y minimización]\label{v1:prop:equiv_max_min}
    Sea el problema de maximización \eqref{v1:eq:problema_max}. Un punto $\bar{x} \in D$ es maximizador de \eqref{v1:eq:problema_max} si y solo si $\bar{x}$ es minimizador del problema equivalente $\min_{x \in D} -f(x)$. Además, se satisface que $\max_{x \in D} f(x) = -\min_{x \in D} (-f(x))$.
\end{proposition}
...
\begin{equation}
    D = \{x \in \Omega \mid h(x) = 0, \; g(x) \le 0\}, \label{v1:eq:restricciones_mixtas_generales}
\end{equation}
