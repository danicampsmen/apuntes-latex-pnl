...
Ahora, por (4.130), (4.131), y la condición de Kummer en $\bar{x}$, podemos tomar la vecindad $U$ tal que, si $x^k \in U$ e $H_k \in \partial_B \Phi(x^k)$, entonces
\begin{align}
...
\begin{equation}
    ...
    \label{eq:4.153}
\end{equation}
donde $A(\bar{x}, \bar{\mu})$ y $B(\bar{x}, \bar{\mu})$ son matrices diagonales $m \times m$ con elementos $a_i = a_i(\bar{x}, \bar{\mu})$ e $b_i = b_i(\bar{x}, \bar{\mu}), i = 1, \dots, m$.
...
\begin{equation}
    ...
    \label{eq:4.157}
\end{equation}
En particular, de (4.155) e (4.157) obtenemos que $\tilde{x} \in \mathcal{K}_+(\bar{x})$.
...
\end{align}
Multiplicando los dos lados en (4.154) por $\tilde{x}$ y utilizando (4.155), (4.157) e (4.158), obtenemos que
\[
    \interno{L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})\tilde{x}}{\tilde{x}} + \sum_{i \in I_0(\bar{x})} \tilde{\mu}_i \interno{g'_i(\bar{x})}{\tilde{x}} = 0.
\]
Llevando en cuenta (4.159), tenemos que
\[
    \interno{L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})\tilde{x}}{\tilde{x}} \le 0,
\]
...
Se para cada iteración $k$ en el método presentado arriba tomamos $H_k \in \partial_B \Phi(x^k, \lambda^k, \mu^k)$ o $H_k \in \partial \Phi(x^k, \lambda^k, \mu^k)$, obtenemos el método de Newton generalizado puro para el sistema KKT. Para el método de Newton generalizado, matrices pueden ser formuladas dadas arriba (vea los ejercicios correspondientes). Esto torna a los métodos en cuestión completamente explícitos e implementables. Resultados de convergencia local y de tasa de convergencia siguen directamente del Teorema 4.5.1 y de las Proposiciones 4.5.1-4.5.3.
...
\begin{theorem}[Convergencia primal superlinear del método de Newton generalizado para el sistema KKT]\label{thm:4.5.3}
...
\begin{proof}
Pela construcción del Algoritmo 4.5.2 y (4.159), para todo $k$,
\begin{equation}
    \Lambda_k(x^{k+1} - x^k) + (h'(x^k))^\top(\lambda^{k+1} - \lambda^k) + (g'(x^k))^\top(\mu^{k+1} - \mu^k) = -L'_x(x^k, \lambda^k, \mu^k),
    \label{eq:4.162}
\end{equation}
\begin{equation}
    h'(x^k)(x^{k+1} - x^k) = -h(x^k),
    \label{eq:4.163}
\end{equation}
\begin{equation}
    -\interno{g'_i(x^k)}{x^{k+1} - x^k} = -\min\{\mu^k_i, -g_i(x^k)\}, \quad i \in J^k_+,
    \label{eq:4.164}
\end{equation}
\begin{equation}
    \mu^{k+1}_i - \mu^k_i = -\min\{\mu^k_i, -g_i(x^k)\}, \quad i \in J^k_-.
    \label{eq:4.165}
\end{equation}
Por las definiciones de $J^k_+$ y de $J^k_-$, las relaciones (4.164) e (4.165) pueden ser escritas como
\begin{equation}
    g_i(x^k) + \interno{g'_i(x^k)}{x^{k+1} - x^k} = 0, \quad i \in J^k_+,
    \label{eq:4.166}
\end{equation}
\begin{equation}
    \mu^{k+1}_i = 0, \quad i \in J^k_-.
    \label{eq:4.167}
\end{equation}
Además de eso, a convergencia de $\{(x^k, \mu^k)\}$ a $(\bar{x}, \bar{\mu})$ implica que
\begin{equation}
...
\begin{align}
    ...
    &= L'_x(\bar{x}, \lambda^{k+1}, \mu^{k+1}) + L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})(x^k - \bar{x}) + o(\|x^k - \bar{x}\|) \nonumber \\
    ...
    \label{eq:4.169}
\end{align}
donde utilizamos a definición de punto estacionario del problema y de multiplicadores de Lagrange asociados, así como a convergencia de $\{(\lambda^k, \mu^k)\}$ a $(\bar{\lambda}, \bar{\mu})$. De (4.169) obtenemos que
\begin{align}
...
Supongamos, primero, que a convergencia de $\{x^k\}$ a $\bar{x}$ sea superlineal, i.e., $\|x^{k+1} - \bar{x}\| = o(\|x^k - \bar{x}\|)$. En este caso, la relación (4.170) se transforma en
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:4.172}
\end{equation}
donde $P_{\mathcal{N}(\bar{x})}((h'(\bar{x}))^\top(\lambda^{k+1} - \bar{\lambda}) + (g'(\bar{x}))^\top(\mu^{k+1} - \bar{\mu})) = 0$. Ahora, de (4.171) obtenemos que
\begin{equation}
...
Llevando en cuenta esta relación, (4.160) se sigue de (4.173).
...
\begin{equation}
    h'(\bar{x})\xi^k = \eta^k, \quad \interno{g'_i(\bar{x})}{\xi^k} = \zeta^k_i \quad \forall i \in J^k_+,
    \label{eq:4.178}
\end{equation}
\begin{equation}
    \|\xi^k\| = o(\|x^{k+1} - x^k\|) + o(\|x^k - \bar{x}\|).
    \label{eq:4.179}
\end{equation}
...
\[
    \interno{(h'(\bar{x}))^\top(\lambda^{k+1} - \bar{\lambda}) + (g'(\bar{x}))^\top(\mu^{k+1} - \bar{\mu})}{d^k} = 0,
\]
donde a última ecuación se sigue de (4.167), (4.176) e (4.178). Multiplicando ambos lados en (4.170) por $d^k$ y utilizando la última ecuación, obtenemos que
\begin{align}
    ...
    \label{eq:4.180}
\end{align}
Pela condición suficiente de segunda orden fuerte (4.146), existe $\gamma > 0$ tal que $\interno{L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})d}{d} \ge \gamma \|d\|^2$ para todo $d \in \mathcal{K}_+(\bar{x})$. De aquí y de (4.180), obtenemos que
\begin{align*}
    \gamma \|d^k\|^2 &\le \interno{L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})d^k}{d^k} \\
    &= \interno{(L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu}) - \Lambda_k)(x^{k+1} - x^k)}{d^k} + \interno{L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})\xi^k}{d^k} + o(\|x^k - \bar{x}\| \|d^k\|) \\
...
\end{align*}
donde a segunda igualdad se sigue de (4.179), y a segunda desigualdad dada sigue del Teorema sobre la proyección sobre un cono convexo y cerrado (Teorema 1.3.1), e la última igualdad se sigue de (4.161). En particular, dividiendo los dos lados de la relación arriba por $\|d^k\|$, concluimos que $\|d^k\| = o(\|x^{k+1} - x^k\|) + o(\|x^k - \bar{x}\|)$.
Por (4.179), tenemos entonces que $\|x^{k+1} - \bar{x}\| = o(\|x^{k+1} - x^k\|) + o(\|x^k - \bar{x}\|)$. Esto significa que existen secuencias $\{t_k\} \to 0$ y $\{s_k\} \to 0$ tales que
\[
...
Observamos que en el caso de la escogencia $\Lambda_k = L''_{xx}(x^k, \lambda^k, \mu^k)$, la convergencia a las variables primales ciertamente es superlineal, ya que $\Lambda_k \to L''_{xx}(\bar{x}, \bar{\lambda}, \bar{\mu})$, y por lo tanto a condición (4.161) vale trivialmente.
\end{proof}
...
A seguir, extenderemos los resultados sobre penalización cuadrática, obtenidos en la Sección 4.4.2 para restricciones de igualdad, al caso de restricciones mixtas del problema (4.181). Para esto, utilizaremos la técnica de introducción de variables de holgura. Acontece que penalización cuadrática, así como Lagrangianas aumentadas (que serán consideradas más adelante), son uno de los pocos casos donde la introducción de variables de holgura es una técnica eficiente, y ella no lleva a resultados más débiles de lo que pueden ser obtenidos considerando desigualdades directamente.
...
\begin{equation}
    ...
    \label{eq:4.183}
\end{equation}
Notemos, primero, que para todo $x \in \Rn$, podemos considerar que la condición $(x, t) \in \tilde{D}$ define el vector $t$ unívocamente, si no nos preocupamos con las señales de las coordenadas del vector $t$. Este punto de vista hace sentido, porque la escogencia de estas señales no tiene ninguna influencia en el valor de la función objetivo del problema (4.182)-(4.183), en el punto $(x, t)$. Por eso, a lo largo del análisis a seguir, vamos a ignorar las señales de las coordenadas de $t$. O sea, cualesquier dos vectores $(x, t^1)$ y $(x, t^2)$, que sean diferentes solamente en signos de las coordenadas de $t^1$ y $t^2 \in \R^m$, serán pensados como ``iguales''. Adoptando este punto de vista, soluciones locales de los mismos problemas, (4.181) y (4.182)-(4.183), unívocamente corresponden un a otro: el punto $\bar{x} \in \Rn$ es una solución local de (4.181) si, y solamente si, el punto $(\bar{x}, \bar{t})$, donde
\begin{equation}
    \bar{t} = (\sqrt{-g_1(\bar{x})}, \dots, \sqrt{-g_m(\bar{x})}),
    \label{eq:4.184}
\end{equation}
está bien definido, y es una solución local del problema (4.182)-(4.183). Más aún, el problema (4.182)-(4.183) no posee otras soluciones locales de forma $(\bar{x}, t), t \in \R^m$.
...
Utilizando el Teorema 4.4.2, obtenemos que $(\bar{x}, t(\bar{x}))$ es un punto estacionario del problema (4.182)-(4.183), y que para una subsecuencia $\{x^{k_j}\}$ convergente a $\bar{x}$ valen las relaciones (4.202) e
\begin{equation}
...
Naturalmente, para el problema (4.182)-(4.183) pueden ser utilizadas otras funciones penalizadas diferenciables, diferentes de aquella introducida. Funciones escogidas resultan en otras funciones penalizadas diferenciables para el problema (4.181). Mencionamos, sin embargo, que no es fácil en este contexto eliminar, al mismo tiempo, tanto a variable auxiliar de holgura $t$, cuanto la variable dual $\mu$. Funciones penalizadas diferenciables para el problema (4.181) que dependen de las variables primal solamente, de hecho existen. Mas la introducción y estudio de estas funciones están fuera del alcance de este apunte.