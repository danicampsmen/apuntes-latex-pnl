...
\begin{algorithm}[Método del gradiente]\label{v2:alg:3.1.1}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \label{v2:eq:3.23}
    \end{equation}
    Por la convexidad de $f$ y por la optimalidad de $\bar{x}$,
    \[
...
Por la estimativa \eqref{v2:eq:3.34}, en la regla de Armijo es preferible elegir el parámetro $\sigma$ próximo a 1/2. Mas incluso para $\sigma = 1/2$, la estimativa \eqref{v2:eq:3.34} es un poco peor de lo que \eqref{v2:eq:3.32} en el caso del método de máximo descenso, así como la estimativa \eqref{v2:eq:3.35}. En relación a esta última, es preferible elegir $\theta$ próximo a 1 (sin embargo, cuanto más próximo está $\theta$ a 1, más cara es la implementación computacional de la regla de Armijo, por lo menos en principio). Finalmente, la elección óptima de la longitud de paso fijo, por la estimativa \eqref{v2:eq:3.36}, viene dada por $\bar{\alpha} = 1/L$ (en este caso, la estimativa \eqref{v2:eq:3.36} coincide con \eqref{v2:eq:3.32}).
